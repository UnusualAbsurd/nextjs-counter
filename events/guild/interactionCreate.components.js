  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId === "showroles") {
      ...
      if (!user)
        return interaction.reply({
          ephemeral: true,
          content: `User has left the server. Couldn't fetch roles.`,
        });
    ...
    if (interaction.customId === "search-command") {
      ...
      interaction.reply({
        ephemeral: true,
        content:
          "You have **30 seconds** to search the commands. Type the command name you would like to search.\nType `end` if you would like to stop.",
      });

      collector.on("collect", (m) => {
        const command =
          client.commands.get(m.content.toLowerCase()) ||
          client.commands.find((a) =>
            a.aliases?.includes(m.content.toLowerCase())
          );
        ...
        if (m.content?.toLowerCase() !== "end") {
          if (!command)
            return m.reply({
              content: `Couldn't find the command \`${m.content}\``,
            });
          if (command) {
            m.reply({
              embeds: [
    ...
    if (interaction.customId === "search-slash") {
      ...
      collector.on("collect", (m) => {
        const command =
          client.slashCommands.get(m.content.toLowerCase()) ||
          client.slashCommands.find((a) =>
            a.name?.includes(m.content.toLowerCase())
          );
        if (m.content?.toLowerCase() === "end") {
          m.reply({ content: "Stopped." });
          collector.stop();
        ...
        if (m.content?.toLowerCase() !== "end") {
          if (!command)
            return m.reply({
              content: `Couldn't find the slash command \`${m.content}\``,
            });
      ...
      const embed = new MessageEmbed()
        ...
        .addFields(
          ...
          },
          {
            ...
            )}:D> [<t:${Math.floor(user.user.createdTimestamp / 1000)}:R>]`,
            inline: true,
          },
          {
            ...
            )}:D> [<t:${Math.floor(user.joinedTimestamp / 1000)}:R>]`,
            inline: true,
          },
    ...
    if (interaction.customId === "viewreason") {
      ...
      if (!data)
        return interaction.reply({
          ephemeral: true,
          content: `Warn ID: \`${text[0]}\` does not exist.`,
        });
      if (data) {
        const moderator =
          (await client.users.fetch(`${data.moderator}`)) || "Deleted User";
        interaction.reply({
          embeds: [
            new MessageEmbed()
              .setColor(client.config.color)
              ...
              .setAuthor(
                ...
                moderator.displayAvatarURL({ dynamic: true })
              )
    ...
    if (interaction.customId === "delwarn") {
      ...
      if (!data)
        return interaction.reply({
          ephemeral: true,
          content: `Warn ID: \`${text}\` does not exist.`,
        });
