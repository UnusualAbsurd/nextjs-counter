...
module.exports = {
  ...
  async execute(client, message, args, errorMsg, prefix) {
    ...
    const placeholder = new MessageActionRow().addComponents([
    ...
    const row = new MessageActionRow().addComponents([
      new MessageButton()
        .setCustomId("search-command")
        .setLabel("Search Command")
        .setStyle("SECONDARY"),
      new MessageButton()
        .setLabel("Search Slash Commands")
        .setCustomId("search-slash")
        .setStyle("SECONDARY"),
    ]);
    ...
    const basic = new MessageEmbed()
      ...
      .addField(
        ...
        [
          ...
          `Invites: [\[ Admin \]](${client.config.invite.admin}) | [\[ Default \]](${client.config.invite.default})`,
        ].join("\n")
      )
      .addField(
        "Information",
    ...
    misc_array.forEach((cmd) =>
      misc_embed.addField(
        `${prefix}${cmd.name}`,
        `<:Reply:887332969033912380> ${cmd.description || "No Description"}`
      )
    );
    info_array.forEach((cmd) =>
      info_embed.addField(
        `${prefix}${cmd.name}`,
        `<:Reply:887332969033912380> ${cmd.description || "No Description"}`
      )
