...
module.exports = {
    ...
    options: [
        {name: 'user', description: "The user that you want to see the warnings of", type: "USER", required: false}
    ],
    /** 
     * @param {Client} client 
    ...
    async execute(client, interaction, args) {
        ...
        if(!data?.length) return interaction.reply  (`**${user.user.tag}** does not have any warnings in this server.`);


        const warns = data.map((w) => {
            const moderator = interaction.guild.members.cache.get(`${w.moderator}`) || "User has left";
            return [
