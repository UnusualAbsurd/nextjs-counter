...
module.exports = {
  ...
  options: [
    {
      ...
      options: [
        ...
        {
          name: "reason",
          description: "The reason why you want to warn this user",
          type: "STRING",
          required: false,
        },
      ],
    }, // Add End
    {
      ...
      options: [
        ...
        {
          name: "reason",
          description: "The reason why you want to delete this warning",
          type: "STRING",
          required: false,
        },
      ],
  ...
  async execute(client, interaction, args) {
    ...
    if (subcommand === "add") {
      if (!interaction.member.permissions.has("MANAGE_MESSAGES"))
        return interaction.reply({
          ephemeral: true,
          content: "Missing Permission `MANAGE_MESSAGES`",
        });
      const data = new db({
        user: user.id,
          ...
          .send({
            embeds: [
              new MessageEmbed()
                ...
                .setTimestamp()
                .setFooter(`${data._id}-${interaction.guildId}`),
            ],
            components: [
    ...
    if (subcommand === "remove") {
      ...
      const data = await db
        .findOne({ guild: interaction.guildId, _id: warnID })
        .catch(() => {});
      if (!data)
        return interaction.reply(
          `Couldn't find a warn with the ID of \`${warnID}\``
        );
