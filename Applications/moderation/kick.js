...
module.exports = {
    ...
    options: [
        { name: 'user', description: "The user that you want to kick", type: "USER", required: true },
        { name: 'reason', description: "The reason that you want to kick the user", type: "STRING", required: false },
    ],
    /** 
     * @param {Client} client 
    ...
    async execute(client, interaction, args, errorMsg) {
        ...
        if (interaction.member.roles.highest.position <= user.roles.highest.position) return errorMsg("You cannot ban users with higher role than you!");
        ...
        modLog(interaction, new MessageEmbed()
            .setColor(client.config.color)
