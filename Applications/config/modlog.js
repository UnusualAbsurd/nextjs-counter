...
module.exports = {
    ...
    async execute(client, interaction, args) {
        ...
        if(data?.modlog === ch.id) return interaction.reply({ ephemeral: true, content: `<#${ch.id}> is already your modlog!` })
        data.modlog = ch.id;
        data.save(function() {
            interaction.reply({
                embeds: [new MessageEmbed().setColor(client.config.color).setDescription(`Successfully set **${message.guild.name}** mod-logs as <#${ch.id}>`).setTimestamp()]
            })
        })
